...
def bootstrap_initial_content():
    ...
    landing_page_list = ContentPage.objects.filter(slug="content")
    if landing_page_list.exists():
        print('blog landing already exists')
        landing_page = landing_page_list.first()
    else:
    ...
    blog_index_list = BlogIndexPage.objects.filter(slug="blog")
    if blog_index_list.exists():
        print('blog index already exists')
        blog_index = blog_index_list.first()
        return
    else:
